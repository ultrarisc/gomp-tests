...
main (int argc, char **argv)
{
  int i, count = 1000000;
  double stime;
  int *unsorted, *sorted, *correct, num_threads;
  if (argc >= 2)
    count = strtoul (argv[1], NULL, 0);

  unsorted = malloc (count * sizeof (int));
  sorted = malloc (count * sizeof (int));
  correct = malloc (count * sizeof (int));
  if (unsorted == NULL || sorted == NULL || correct == NULL)
    {
  ...
  memcpy (correct, unsorted, count * sizeof (int));
  quickSort(correct, 0, count-1);
  ...
  more_verify("sort1", sorted, correct, count);

  memcpy (sorted, unsorted, count * sizeof (int));
  stime = omp_get_wtime ();
  sort2 (sorted, count);
  verify ("sort2", stime, sorted, count);
  more_verify("sort2", sorted, correct, count);
  ...
  more_verify("sort3", sorted, correct, count);
#endif
...
void more_verify(char *what, int *result, int *correct, int count) {
  for (int i = 0; i < count; ++i) {
    if (result[i] != correct[i]) {
      printf("[%s] wrong number at %d: %d should be %d\n", what, i, result[i], correct[i]);
    }
  }
}